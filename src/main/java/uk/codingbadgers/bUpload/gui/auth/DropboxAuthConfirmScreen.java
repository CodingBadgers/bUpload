...
public class DropboxAuthConfirmScreen extends bUploadGuiScreen {
    private static final int LOG_OUT = 0;
    private static final int CANCEL = 1;

    private GuiScreen screen;

    public DropboxAuthConfirmScreen(bUploadGuiScreen auth, GuiScreen screen) {
        super(auth);
        this.screen = screen;
    }
    ...
    public void drawScreen(int par1, int par2, float par3) {
        this.drawBackground();
        drawCenteredString(this.fontRendererObj, EnumChatFormatting.UNDERLINE + TranslationManager.getTranslation("image.dropbox.title"), this.width / 2, this.height / 6 + 30, 0xFFFFFF);
        drawCenteredString(this.fontRendererObj, TranslationManager.getTranslation("image.confirm.ln1", EnumChatFormatting.GOLD + DropboxAuthHandler.getInstance().getUsername()), this.width / 2, this.height / 6 + 42, 0xFFFFFF);
    ...
    protected void actionPerformed(GuiButton button) {
        switch (button.id) {
            case LOG_OUT:
                try {
                    DropboxAuthHandler.getInstance().forgetData();
                } catch (IOException e) {
                    e.printStackTrace();
                }
