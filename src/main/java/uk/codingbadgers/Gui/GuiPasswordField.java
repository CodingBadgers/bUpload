...
public class GuiPasswordField extends Gui {
    /**
     * Have the font renderer from GuiScreen to render the textbox text into
     * the screen.
     */
    ...
    /**
     * The current character index that should be used as start of the
     * rendered text.
     */
    ...
    /**
     * Moves the text cursor by a specified number of characters and clears
     * the selection
     */
    ...
    public void drawTextBox() {
        if (this.getVisible()) {
            ...
            String s = this.fontRenderer.trimStringToWidth(this.text.substring(this.lineScrollOffset), this.getWidth());
            s = s.replaceAll("(?s).", "*"); // CodingBadgers - replace all characters with '*', to hide password
    ...
    /**
     * the side of the selection that is not the cursor, maye be the same as
     * the cursor
     */
